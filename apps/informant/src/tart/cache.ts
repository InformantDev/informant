...
async function pruneCacheVersions(
  parent: string,
  current: string,
): Promise<void> {
  ...
  for (const entry of entries) {
    ...
    const metadata = await stat(join(parent, entry.name)).catch(
      () => undefined,
    );
    if (metadata)
      versions.push({ name: entry.name, modifiedAt: metadata.mtimeMs });
  }
  ...
  await Promise.all(
    versions
      ...
      .map((version) =>
        rm(join(parent, version.name), { recursive: true, force: true }),
      ),
  );
}
...
export async function cacheMounts(
  ...
  if (!job.cache)
    return {
      args: [] as string[],
      mounts: [] as Array<{ name: string; path: string; readOnly?: boolean }>,
      restore: "",
      save: "",
  ...
  for (const cache of job.cache) {
    ...
    for (const keyFile of cache.keyFiles) {
      ...
      if (!metadata.isFile() || metadata.size > 16 * 1024 * 1024) {
        throw new Error(
          `cache key file must be a regular file no larger than 16 MiB: ${keyFile}`,
        );
      }
    ...
    const cacheKey =
      cache.keyFiles.length > 0 ? key.digest("hex").slice(0, 24) : "default";
        ...
        const host =
          trusted && !cache.buildScoped
            ...
            : join(
                workspace,
                "..",
                cache.protectedChannel
                  ? "protected-shared-caches"
                  : "shared-caches",
                cachePathIdentity(user, path),
              );
        await mkdir(host, { recursive: true });
        const resolvedHost = await realpath(host);
        args.push(`--dir=cache-${mountIndex}:${resolvedHost}`);
        mounts.push({
          name: `cache-${mountIndex}`,
          path: resolvedHost,
          ...(cache.readOnly ? { readOnly: true } : {}),
        });
        if (!cache.readOnly) writablePaths.push(resolvedHost);
