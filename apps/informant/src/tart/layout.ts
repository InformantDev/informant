...
export function bunCopyfileBackend(lockDirectory?: string): string {
  ...
  const release = lockDirectory
    ...
    : "";
  return `bun() { case "$1" in install|i|add|remove|update|link|unlink) ${acquire}for arg in "$@"; do case "$arg" in --backend|--backend=*) command bun "$@"${release}; return;; esac; done; command bun "$@" --backend=copyfile${release};; *) command bun "$@";; esac; }; export -f bun;`;
}
